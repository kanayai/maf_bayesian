...
### [`analyze_residuals.py`](file:///Users/kai21/Projects/Research/certest/maf_bayesian/analyze_residuals.py)
Performs **Residual Analysis** to validate the measurement error model.

*   **Residual Calculation**: Computes residuals ($y_{obs} - \mu_{post}$) at experimental data points.
*   **Standardization**: Calculates standardized residuals using the load-dependent noise variance ($\sigma^2 \propto P$).
*   **Visualization**: Generates diagnostic plots, including **Raw Residuals with Uncertainty Bands** and **Standardized Residuals vs Load**, to check for homoscedasticity and model adequacy.

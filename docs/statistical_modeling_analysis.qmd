---
title: "Statistical Modeling Analysis: posterior_no_bias.py"
format:
  html:
    theme: cosmo
    toc: true
...
- **Type**: Gaussian Process Emulator with Multi-Output (Horizontal & Vertical).
- **Goal**: To predict the system response $y$ (extension) given inputs $x$ (load, angle) and parameters $\theta$ (E1, E2, etc.).
- **Structure**:
  ...
  - $\epsilon \sim \mathcal{N}(0, \sigma^2_{measure})$: Random measurement noise.
...
The model assumes conditional independence between Horizontal ($H$) and Vertical ($V$) outputs given the parameters:
...
- **Action**: It takes the "learned" parameters from the MCMC results (`idata`) and uses them to predict the material behavior at a specific test angle (e.g., 45°).
- **Method**:
  ...
  3.  **Aggregation**: It computes the mean and 90% confidence intervals (5th and 95th percentiles) of these predictions.

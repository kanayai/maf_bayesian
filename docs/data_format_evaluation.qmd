# Data Format Evaluation: HDF5 vs. NetCDF (ArviZ)


## Current Workflow (HDF5)
The current workflow in `analyze.py` primarily uses HDF5 / NetCDF to store and retrieve MCMC samples.
...
While HDF5 is sufficient for raw storage, adopting ArviZ `InferenceData` (saved as NetCDF) will significantly streamline post-processing in `analyze.py`. It eliminates the need for manual array slicing and boilerplate plotting code, and provides robust diagnostics out of the box.

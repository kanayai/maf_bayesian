...
*   **Global Slope** ($\mu_{emulator}$): The foundational stiffness/compliance parameter share across all experiments.
    $$ \mu_{emulator} \sim \text{LogNormal}(\ln(0.01), 1) $$
    *   **Prior**: LogNormal distribution with `log_mean` = $\ln(0.01) \approx -4.605$ and `log_scale` = 1.
    *   **Interpretation**: Median slope of $0.01$ (corresponding to 100 GPa stiffness if area=1), with a wide multiplicative variance (1-sigma range approx $0.0037$ to $0.027$).

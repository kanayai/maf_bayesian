...
| **Mean Function** | $P \sin(\alpha)$ OR $P \cos(\alpha)$ | $P \sin(\alpha) \cdot \mu_h$ AND $P \cos(\alpha) \cdot \mu_v$ |
...
| :--- | :--- | :--- |
| **$\mu_{emulator, v}$** | GP Mean Offset (Normal/Vertical) | $\mathcal{N}(0, 0.01)$ |
| **$\mu_{emulator, h}$** | GP Mean Offset (Shear/Horizontal) | $\mathcal{N}(0, 0.01)$ |
...
| **$\gamma_{scale, v}$** | Gamma Scale (Normal) | $\text{Exponential}(5.0)$ |
| **$\gamma_{scale, h}$** | Gamma Scale (Shear) | $\text{Exponential}(10.0)$ |
...
| **Slope Bias** ($b_{slope}$) | $\text{Exponential}(5000)$ | $0.0002$ | Accounts for experiment-specific deviations in the linear slope (empirical model). |

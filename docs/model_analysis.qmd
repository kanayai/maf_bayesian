...
1.  Sample a latent offset:
    83000 \epsilon_i \sim \mathcal{N}(0, \sigma_{\text{latent}}^2) 83000
    where $\sigma_{\text{latent}} \sim \text{LogNormal}(-5, 1.0)$ (prior mean $\approx 0.01$).

2.  Modify the Mean Function:
    83000 \boldsymbol{\mu}_{total}(\mathbf{x}_i, \theta) = \boldsymbol{\mu}_{physics}(\mathbf{x}_i, \theta) + \epsilon_i 83000

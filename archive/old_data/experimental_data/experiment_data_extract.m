...
%         end
        ...
        temp = file_names_o(15:n_sep:end); % start from index 15
        file_names = [file_names_o(1); temp];
        temp = loads_o(15:n_sep:end);
...
%         extensions = [extensions; zeros(length(angles(ind_not)),3)];
        ...
        writematrix(extensions, ['data_extension_exp_' num2str(angles(ind_angles)) '_' direction '_' num2str(ind_data) '.txt']);
        writematrix(loads_angle, ['input_load_angle_exp_' num2str(angles(ind_angles)) '_' direction '_' num2str(ind_data) '.txt']);
        ind_data = ind_data + 1;
    end

...
# Pattern for bias_E1 results. 
# Note: If you haven't run with bias_E1 yet, this might not find files. 
# But for the purpose of this script, we assume we want to analyze bias_E1 runs.
# If you want to test with no_bias files, change this pattern.
pattern = "bias_E1_hv_*_MAF_linear.nc" 
files = list(results_dir.glob(pattern))
if not files:
    # Fallback to no_bias if bias files not found, just to allow script to run/test logic
    # But warn the user
    print("Warning: No bias_E1 files found. Falling back to no_bias files for testing.")
...
fig_prior.savefig(fig_path.joinpath("prior_prediction_" + str(angle_value) + "deg_linear_bias_E1_" + suffix + "." + fig_format), dpi=dpi, transparent=True)
...
fig_post.savefig(fig_path.joinpath("post_prediction_"+str(angle_value)+"deg_linear_bias_E1_" + suffix + "." + fig_format), dpi=dpi, transparent=True)
...
fig_all.savefig(fig_path.joinpath("prediction_"+str(angle_value)+"deg_linear_bias_E1_" + suffix + "." + fig_format), dpi=dpi, transparent=True)
